...
default_intercept_path_glob() ->
    filename:join([intercepts_local:home_dir(), "intercepts", "*.erl"]).
...
load_intercepts(Nodes, Globs) ->
    case intercepts_config:get(load_intercepts, true) of
        false ->
            ok;
        true ->
            Intercepts = intercepts_config:get(intercepts, []),
...
%% requires use of the intercepts_pt parse transform.
