...
%% `intercepts_redbug:trace/2` that are in the test suites. The traces
%% are printed to the test.log
%%
%% -------------------------------------------------------------------
-module(intercepts_redbug).
...
set_tracing_applied(TracingApplied) when is_boolean(TracingApplied) ->
    ...
    Enabled = case TracingApplied of
        true -> TracingApplied;
        _ -> intercepts_config:get(apply_traces, false)
    end,
    case Enabled of
        true ->
    ...
    intercepts_config:set(apply_traces, Enabled).

is_tracing_applied() ->
    intercepts_config:get(apply_traces, false).
...
%%     intercepts_redbug:trace(Node, "riak_kv_qry_compiler:compile").
%%     intercepts_redbug:trace(Node, ["riak_kv_qry_compiler:compile", "riak_ql_parser:parse"]).
%%
%% Multiple traces can be set in one call. Calling the `intercepts_redbug:trace`
%% function a second time stops the traces started by the first call.
%%
%% Traces can be stopped by calling `intercepts_redbug:stop(Nodes)` with the nodes in
